...
type Exam = { id: string; code: string; name: string; active: boolean };
...
const EMPTY_FORM: FormValues = { code: "", name: "", active: true };
...
export default function ExamsPage() {
  ...
  function openEdit(e: Exam) {
    setEditing(e);
    form.reset({ code: e.code, name: e.name, active: e.active });
    setDialogOpen(true);
  }
  ...
  function exportCsv() {
    const rows = [
      ["Code", "Name", "Active"],
      ...exams.map((e) => [e.code, e.name, e.active] as const),
    ];
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                exams.map((e) => (
                  <tr key={e.id} className="border-b last:border-0 even:bg-slate-50/50">
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Field label="Name" error={form.formState.errors.name?.message}>
              <Input placeholder="e.g. Internist" {...form.register("name")} />
            </Field>
