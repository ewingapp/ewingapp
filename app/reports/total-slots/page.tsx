...
type SortKey = "doctor" | "total" | "scheduled" | "pct";
...
export default function TotalSlotsReportPage() {
  const today = ptTodayIso();
  const [fromDate, setFromDate] = useState<string>(today);
  const [toDate, setToDate] = useState<string>(() => addDaysIso(today, 7));
  ...
  function downloadCsv() {
    ...
    const head = ["Doctor", "Total Slots", "Scheduled", "% Filled"];
    ...
    a.href = url;
    a.download = `total-slots-${fromDate}_${toDate}.csv`;
    a.click();
    ...
    <AppShell>
      <div className="max-w-[1000px] mx-auto px-6 py-6">
        ...
        <h1 className="text-2xl font-semibold text-slate-900 mb-1">
          Total Slots and Scheduled Slots Report
        </h1>
        <p className="text-sm text-slate-600 mb-6">
          A count of all total slots created and total scheduled appointments
          per doctor for a date range.
        </p>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-100 text-slate-700">
                    <tr>
                      ...
                      <Th sortKey="scheduled" current={sortKey} dir={sortDir} onSort={onSort}>
                        Scheduled
                      </Th>
